// ===== GoodEnergy App Logic (v2 - com simulação de API e melhorias de UX) =====

// Persistent user mock
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            if (shouldFail) {
                ...
                resolve(JSON.parse(JSON.stringify(data))); // Clona o objeto para evitar mutação direta
            }
        }, delay);
    });
}
...
function showToast(message, type = 'success') {
    ...
    setTimeout(() => {
        toast.remove();
    }, 4000); // Remove o toast após 4 segundos
}
...
async function renderDevices() {
    ...
    try {
        ...
        tbody.querySelectorAll('button[data-tgl-id]').forEach(btn => btn.addEventListener('click', async e => {
            ...
            dev.on = !dev.on; // Simula a ação antes da chamada de API para feedback rápido
            ...
            try {
                // A chamada de "API" real aconteceria aqui para persistir a mudança
...
function loginInit(){
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await simulateApiCall({ token: 'fake_jwt_token' }); // Simula o login
  ...
  try {
    ...
    el('userSave').addEventListener('click', async () => {
        ...
        try {
            ...
            el('userName').textContent = nu.name; // Atualiza o nome na sidebar
        } catch (error) {
...
function reportsInit() {
    ...
    const generateReportChart = async () => {
        try {
            // Simula a busca de dados para o relatório
