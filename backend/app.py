...
def get_battery_status():
    ...
    """
    try:
        # Gera dados aleatórios para a simulação
        charged_percentage = random.randint(1, 100)
        empty_percentage = 100.0 - charged_percentage
        now = datetime.now()
        # Simula que a bateria carrega durante o dia (9h-16h)
        is_charging = 9 <= now.hour < 16
        if is_charging:
            fluxo_watts = random.randint(500, 2500) # Fluxo positivo = carregando
            status_texto = "Carregando"
        else:
            fluxo_watts = random.randint(-1500, -300) # Fluxo negativo = descarregando
            status_texto = "Descarregando"
        
        capacidade_total_kwh = 13.5 # Capacidade total fixa para o cálculo
